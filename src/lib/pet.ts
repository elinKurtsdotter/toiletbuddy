...
export function accidentFreeStreak(state: AppState): number {
  ...
  if (accidentDays.size === 0) {
    ...
    const firstVisitDay = dayKey(state.visits[0].ts);
    const today = dayKey(Date.now());
    return daysBetween(firstVisitDay, today) + 1;
  }
...
export function streakStage(state: AppState): EvolutionStage {
  ...
  console.log("streakStage debug:", {
    threshold,
    streak,
    maxEvolutionStage: state.maxEvolutionStage,
  });
